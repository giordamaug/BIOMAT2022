...
def do_cv(nfolds, nclasses, seed):
  ...
  for fold, (train_idx, test_idx) in enumerate(tqdm(kf.split(np.arange(len(X)), y), desc=bcolors.OKGREEN + f'{seed+1}-iter', total=nfolds)):
      ...
      scores = pd.concat([scores, pd.DataFrame([[roc_auc_score(test_y, preds) if nclasses <=2 else np.nan, accuracy_score(test_y, preds), balanced_accuracy_score(test_y, preds), 
