...
df_net.loc[df_net["IsMetabolic"]].drop(columns=['IsPhysical', 'IsMetabolic', 'link']).to_csv("met_edges.csv")
df_net.loc[df_net["IsPhysical"]].drop(columns=['IsPhysical', 'IsMetabolic','link']).to_csv("ppi_edges.csv")
