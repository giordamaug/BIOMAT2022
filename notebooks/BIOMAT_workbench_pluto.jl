...
@bind dataset Select(["LUNG", "KIDNEY"])

# ╔═╡ be213a72-b268-4f36-9860-d4c70becf9d9
@bind path TextField((30,1); default="/Users/maurizio/BIOMAT2022")


# ╔═╡ 395da9a0-e72e-4865-9985-e97d6b6d0c20
md"""
## Loading gene attributes
It removes possible duplicated genes
"""

# ╔═╡ cf093a50-77c7-11ed-1495-75cf955da2e0
function read_attributes(path, dataset, filename, middledir="") 
	df = CSV.File("$path/$dataset/$middledir/$filename.csv", typemap=Dict(Int => Float64)) |> DataFrame      # read the attribute file
	df = unique(df, :name)      # remove possible duplicated genes
	return df
end

# ╔═╡ 178e3a94-c87c-482e-840c-cb553ef710d4
md"""
Remove non-numeric columns, replace all NaNs and missing values to mean value in column
"""
...
function load_data()
	...
	df[!, :name] = genes
	dflabel = read_attributes(path, dataset, "node_labels")
...
begin
	...
	df = load_data()
	display(first(df,5))
	X = Matrix(df[!, Not(r"name|most_freq")])
	y = vec(Matrix(DataFrames.select(df, [:most_freq])))
	rng = StableRNG(566)
    ...
    for s in splits
        ...
        targets = cat!(vec(y[test, :]), dims=1)
        preds = cat!(MLJ.predict_mode(mach, rows=test), dims=1)
    end
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "230b9dd27006ef370e6957ee9ae4c2745d3f7d8e"
